...
class SubmissionMail extends Mailable
{
    ...
    public function __construct($mailData,$modulename,$final)
    {
        ...
        $MailrecipientNoBu = DB::table('tbl_mailrecipient')
        ->where('module',$modulename)
        ->where('isActive',1)
        ->get();

        // Base query for mail recipients
        $queryMr = DB::table('tbl_mailrecipient')
            ->where('module', $modulename)
            ->where('isActive', 1);
        ...
        if (isset($mailData['submission']->employee_id)) {
            ...
            $companyCode = DB::table('tbl_employee')
        ...
        // Jdi MODULE
            if($modulename == 'Jdi') {
                ...
                if($final == 1) {
                    ...
                    $this->attach($url."devjobportal/".$pdf); // add attachment to mail
                    foreach ($Mailrecipient as $cc) {
